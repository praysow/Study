...
pipe = Pipeline([('MinMax',MinMaxScaler()),('RF',RandomForestClassifier())])                  #전부소문자에 언더바 두개를 붙여준다
# pipe = make_pipeline(MinMaxScaler(),HalvingGridSearchCV(RandomForestRegressor(),param_grid={"n_estimators": [5],"n_jobs": [2]},cv=5))

model1 = GridSearchCV(pipe,param_grid,cv=5,verbose=1)
