...
model1 = GridSearchCV(RandomForestClassifier(),parameters, cv = kfold,verbose=1,refit=True,n_jobs=-1)       #n_jobs gpu아니고 cpu
model2 = RandomizedSearchCV(RandomForestClassifier(),parameters, cv = kfold,verbose=1,refit=True,n_jobs=-1)       #n_jobs gpu아니고 cpu
s_t= time.time()
model1.fit(x_train,y_train)
e_t= time.time()
print("최적의 매개변수",model1.best_estimator_)         
# 최적의 매개변수 SVC(C=10, kernel='linear')
print("최적의 파라미터", model2.best_params_)
# 최적의 파라미터 {'C': 10, 'degree': 3, 'kernel': 'linear'}
print("베스트 스코어", model1.best_score_)
#베스트 스코어 0.9916666666666666
print("model 스코어", model1.score(x_test,y_test))
# model 스코어 0.9333333333333333
y_pred = model1.predict(x_test)
print("acc",accuracy_score(y_test,y_pred))
#acc 0.9333333333333333
y_pred_best = model1.best_estimator_.predict(x_test)
...
'''
베스트 스코어 0.78912391089837
model 스코어 0.8049844236760124
