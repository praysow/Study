...
model = HalvingRandomSearchCV(RandomForestClassifier(),parameters, cv = kfold,verbose=1,refit=True,n_jobs=-1,random_state=6,factor=3.5,min_resources=10)     #데이터를 최대한 사용하고싶다면 factor와,min_resources 를 조절하자
# model = HalvingGridSearchCV(RandomForestClassifier(),parameters, cv = kfold,verbose=1,refit=True,n_jobs=-1,random_state=6,factor=2,min_resources=16)     #데이터를 최대한 사용하고싶다면 factor와,min_resources 를 조절하자
