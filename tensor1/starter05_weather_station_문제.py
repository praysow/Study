...
#         tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(8, input_shape=(N_PAST, N_FEATURES), return_sequences=True)),
...
def solution_model():
    ...
    model.compile(loss='mae', optimizer=optimizer)
    model.fit(train_dataset, epochs=100)
    mae = model.evaluate(valid_dataset)
...
if __name__ == '__main__':
    ...
    # model.save("c5q2.h5")
'''
MAE on validation set: 0.3027242124080658
'''