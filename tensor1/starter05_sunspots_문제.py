...
def solution_model():
    ...
    with open('sunspots.csv') as csvfile:
        ...
        next(reader)
        for row in reader:
            sunspots.append(float(row[2]))  # Extract sunspots data
            time_step.append(int(row[0]))    # Extract time step

    series = np.array(sunspots)  # Convert sunspots list to numpy array
