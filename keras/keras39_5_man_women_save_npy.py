...
path_train= "c:/_data/kaggle/man_women/data"
...
# print(xy_train)
test=test_datagen.flow_from_directory(path_test,target_size=(batch1,batch2),batch_size=1600,class_mode='binary')            #원본데이터는 최대한 건드리지 말자,원본데이터는 각각다르니 target_size를 통해서 사이즈를 동일화시킨다
...
# x_test = xy_test[0][0]
# y_test = xy_test[0][1]
# print(xy_train[0][0].shape,xy_train[0][1].shape)  #(1600, 100, 100, 3) (1600,)
# print(test.shape)
...
np.save(np_path + 'keras39_5_x_train.npy', arr=x)
np.save(np_path + 'keras39_5_y_train.npy', arr=y)
np.save(np_path + 'keras39_5_x_test.npy', arr=test)
