...
def objective(trial):
    ...
    lgbm_params = {
        ...
        "verbosity": -1,
        "boosting_type": "gbdt",
        "random_state": trial.suggest_int("random_state", 500, 1000),
    ...
    # LightGBM 모델 생성
    model = lgb.LGBMRegressor(**lgbm_params,device='gpu')
...
best_model.booster_.save_model("c:/_data/dacon/soduc/weight/money3_lgb_optuna.csv")

# 테스트 데이터 예측 및 저장
y_pred_test = best_model.predict(test)
sample['Income'] = y_pred_test
sample.to_csv("c:/_data/dacon/soduc/csv/money3_lgb_optuna.csv", index=False)
print('Best parameters:', best_params)
